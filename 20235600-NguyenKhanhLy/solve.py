...
def predict_message(plaintext: str) -> str:
    """
    Try to automatically predict the intended message by filling underscores
    or gibberish parts with likely English words or spacing patterns.
    This is a heuristic human-like guess, not guaranteed to be perfect.
    """
    import re

    # Replace long underscore sequences with likely words/phrases
    text = plaintext

    # Heuristic replacements (can extend this dictionary)
    common_fixes = {
        r"_+": " ",                # replace underscores with spaces
        r"\bteh\b": "the",         # common typo correction
    ...
    # If text ends mid-sentence, guess a plausible ending
...
# -----------------------
# Manual hints utilities
# -----------------------
# manual hint format: list of tuples (cipher_index, position, character)
# cipher_index: 0..len(ciphertexts)-1 ; position: int ; character: single-character string
MANUAL_HINTS: List[Tuple[int, int, str]] = [
    # example hints you can edit/add
    (0, 7, 'f'),
    (0, 18, 'n'),
    (1, 25, 'y'),
    (4, 56, 'p'),
...
def apply_manual_hints(key: List[Optional[int]], ciphertexts: List[bytes], hints: List[Tuple[int, int, str]]) -> None:
    """Apply human-provided plaintext character hints to derive key bytes."""
    for ci, pos, ch in hints:
        if ci < 0 or ci >= len(ciphertexts):
            continue
        if pos < 0 or pos >= len(ciphertexts[ci]):
            continue
        key_byte = ciphertexts[ci][pos] ^ ord(ch)
        key[pos] = key_byte
...
def main() -> None:
    ...
    # initial evidence and derive key via space heuristic
    ...
    # initial refinement by printability
    ...
    # show partial decryption before manual hints
    print("\n--- Partial decryption BEFORE manual hints ---")
    for idx, c in enumerate(ciphertexts[:-1], start=1):
        print(f"Ciphertext #{idx} plaintext: {decrypt_with_key(c, key)}")
    print("\nTarget (raw):", decrypt_with_key(target, key))

    # Apply manual hints (you can edit MANUAL_HINTS at top)
    if MANUAL_HINTS:
        print("\nApplying manual hints...")
        apply_manual_hints(key, ciphertexts, MANUAL_HINTS)

        # after applying hints, refine again
        refine_key_by_printability(
            ciphertexts, key, space_evidence, evidence_threshold, min_printable_ratio=0.75, max_passes=2
        )

        print("\n--- Partial decryption AFTER manual hints ---")
        for idx, c in enumerate(ciphertexts[:-1], start=1):
            print(f"Ciphertext #{idx} plaintext: {decrypt_with_key(c, key)}")
        print("\nTarget (raw, after hints):", decrypt_with_key(target, key))
    else:
        print("\nNo manual hints supplied (MANUAL_HINTS is empty).")

    # Final predicted message (auto-clean)
    plaintext = decrypt_with_key(target, key)
    predicted = predict_message(plaintext)
    print("\n--- Auto-decrypted (raw final) ---")
    print(plaintext)
    print("\n--- Predicted (cleaned / human-like) ---")
    print(predicted)

    with open("secret.txt", "w", encoding="utf-8") as f:
        f.write(predicted + "\n")
    print("\nSaved predicted plaintext to secret.txt")
